...
| W05 ~ W52 | 1~4 | B1~C1 | 분기별 주제 로드맵 | 발행 예정 |
...
- **본문으로 바로가기** 스킵 링크 — Tab 을 누르면 나타나 본문으로 바로 건너뜁니다
- 모바일 햄버거 메뉴(한 항목당 48px 이상), 스크롤 시 헤더 경계선, 맨 위로 가기, 키보드 포커스 링
...
한 화면에 1년치가 들어 있는 긴 문서라, "지금 어디에 있는지 + 다음에 어디로 갈지"를 항상 보이게 했습니다.

- **빠른 이동 칩 바(바로 가기)** — 헤더 아래에 붙어 따라오는 칩 줄입니다.
  이번 주 · 섹션 · 복습 · 단어장 · 플랜 · 알림 신청으로 한 번에 이동하고,
  스크롤 위치에 따라 지금 보고 있는 화면이 강조됩니다(헤더 메뉴·하단 탭바와 동기화 — `script.js`의 스크롤 스파이).
  좁은 화면에서는 줄바꿈 대신 옆으로 스크롤되고, 오른쪽 끝이 흐려 "뒤에 칩이 더 있다"를 알려 줍니다.
- **모바일 하단 탭바** — 760px 이하에서만 나타나는 고정 탭바입니다.
  홈 · 이번 주 · 복습 · 단어장 · 플랜이 엄지가 닿는 아래쪽에 놓이고, 스크롤 위치에 따라 현재 화면이 자동 강조됩니다.
  iOS 홈 인디케이터 영역은 `env(safe-area-inset-bottom)`으로 비워 두고, 인쇄할 때는 숨깁니다.
- **목차(TOC) 스크롤 스파이** — 사이드바 목차에서 지금 읽는 섹션이 강조되고, 완료한 섹션은 표시가 남습니다.
- **섹션 끝 "다음 섹션" 링크** — 본문을 다 읽으면 바로 다음 섹션(마지막이면 복습)으로 이어집니다.
- 앵커 이동은 `scroll-padding-top`(헤더 + 칩 바 높이)만큼 띄워, 읽던 자리가 상단 바에 가려지지 않습니다.

## 모바일 최적화

- 터치 영역 최소 44px — 버튼·칩·받아쓰기 입력 등 주요 조작 요소를 손가락으로 잘못 짚지 않게 합니다
- 입력 글자 16px — iOS 사파리가 초점을 줄 때 화면을 확대하지 않습니다
- 하단 탭바·맨 위로·토스트 알림이 겹치지 않도록 safe-area 만큼 함께 올라갑니다
- 주(week) 선택기 칩은 좁은 화면에서도 52주가 한눈에 보이게 줄바꿈으로 배치합니다
